...
本ドキュメントは、システム全体のレイヤー構成と設計方針を定める。
...
- 全レイヤーに共通する基盤のみを管理する
  - 認証基盤（Cognito）。ログイン・サインアップ・パスワード変更の UI は Cognito Managed Login を利用し、フロントエンド側での認証画面の実装を省く
  - フロントエンド配信（S3）
...
| バックエンド | API Gateway + Lambda (SAM) | サーバーレスで運用負荷が低い。SAM により Lambda + API Gateway を一体管理できる |
