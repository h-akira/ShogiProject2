...
#### F-KIFU-01: マイページ（棋譜一覧）

- ログイン後の起点となるダッシュボード画面
- **概要情報**: 保存されている棋譜の総数を表示する
- **最近の更新**: 最終更新が新しい順に最大10件の棋譜を表示する
- 各棋譜にはスラグ、先後、勝敗、付与されたタグが表示される
- タグはクリック可能で、クリックするとタグ別棋譜一覧（F-TAG-05）に遷移する
...
- この画面からタグ名の編集（F-TAG-03）およびタグの削除（F-TAG-04）を実行できる
...
graph TD
  ...
  KifuList --> KifuDetail
  KifuList --> KifuCreate
  KifuList -->|タグクリック| TagDetail
  KifuDetail --> KifuEdit
  KifuDetail -->|タグクリック| TagDetail
  Explorer --> KifuDetail
  TagList --> TagCreate
  TagList --> TagDetail
  TagList --> TagEdit
