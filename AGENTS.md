...
- **Markdown 内の AI 指示コメントを確認する**: Markdown ファイル内に HTML コメント（`<!-- ... -->`）がある場合、AI 向けの編集指示が含まれている可能性がある。指示に従うこと。AI 指示コメントには 2 種類ある:
  - `<!-- AI指示: ... -->`: 対応完了後にコメントを削除する
  - `<!-- AI指示（削除不可）: ... -->`: 対応後もコメントを削除してはならない（永続的なルール）
  - **先頭行（1 行目）の AI 指示**: ファイル全体に適用されるルールであるため、編集箇所に関わらずファイルを開いたら必ず確認すること
  - **先頭行以外の AI 指示**: 関連する箇所を編集する場合のみ確認・対応すればよい
